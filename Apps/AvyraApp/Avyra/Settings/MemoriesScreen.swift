...
private struct MemoryRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 8) {
                ...
                Text(Self.relativeDateLabel(self.item.createdAt))
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                if self.settings.developerModeEnabled {
                    // UUID is developer noise — only useful when you
                    // need to correlate a row with a log line or
                    // GRDB query.
                    Text(self.item.id)
    ...
    /// "5 min ago" / "yesterday" / "Nov 12" — friendly recency that
    /// scales naturally from seconds to weeks.
