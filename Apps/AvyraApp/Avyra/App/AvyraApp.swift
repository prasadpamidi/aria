...
struct AvyraApp: App {
    ...
    init() {
        ...
        // saved retention preferences from `AvyraSettings` (which
        ...
        if case let .success(storage) = result {
            ...
            // `AvyraSettings`) — App.init runs before the SwiftUI
    ...
    private let sharedModelContainer: ModelContainer = {
        ...
        // SwiftData side of the app is unusable, but the Aria-backed chat keeps
    ...
    @ViewBuilder private var rootView: some View {
        ...
        case let .failure(error):
            VStack(spacing: 12) {
                ...
                Text("Storage failed to initialize")
                    .font(.headline)
    ...
    /// Open (or create) the Avyra database in Application Support.
    ...
    private static func loadStorage() -> Result<GRDBStorage, any Error> {
        Result {
            ...
            // Filename stays `aria-sample.sqlite` for backwards
            // compatibility with any existing on-device DB from prior
            // builds. Renaming would orphan that data.
